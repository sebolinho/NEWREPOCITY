...
class WatchController extends Controller
{
    ...
    public function tv(Request $request, $slug)
    {
        try {
            ...
            \Log::warning('TV show not found: ' . $slug, [
                'slug' => $slug,
                'user_id' => auth()->id()
            ]);
            abort(404);
        } catch (\Exception $e) {
            \Log::error('Error loading TV show page: ' . $e->getMessage(), [
                'slug' => $slug,
                'user_id' => auth()->id(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function episode(Request $request, $slug, $season, $episode)
    {
        try {
            ...
            $episode = PostEpisode::where('post_id', $listing->id)->where('status', 'publish')->where('season_number',
                $season)->where('episode_number', $episode)->firstOrFail();
            ...
            if ($request->user() and !$episode->logs()->exists() and config('settings.history') == 'active') {
                try {
                    ...
                    \Log::warning('Error tracking episode view: ' . $e->getMessage(), [
            ...
            \Log::warning('Episode not found: ' . $slug . ' S' . $season . 'E' . $episode, [
                'slug' => $slug,
                'season' => $season,
                'episode' => $episode,
                'user_id' => auth()->id()
            ...
            \Log::error('Error loading episode page: ' . $e->getMessage(), [
                'slug' => $slug,
                'season' => $season,
                'episode' => $episode,
                'user_id' => auth()->id(),
    ...
    {
        try {
            $listing = Broadcast::where('slug', $slug)->firstOrFail();
            ...
            \Log::warning('Broadcast not found: ' . $slug, [
                'slug' => $slug,
                'user_id' => auth()->id()
            ]);
            abort(404);
        } catch (\Exception $e) {
            \Log::error('Error loading broadcast page: ' . $e->getMessage(), [
                'slug' => $slug,
                'user_id' => auth()->id(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    {
        try {
            $listing = PostVideo::where('id', $slug)->firstOrFail();
            ...
            \Log::warning('Embed video not found: ' . $slug, [
                'slug' => $slug,
                'user_id' => auth()->id()
            ]);
            abort(404);
        } catch (\Exception $e) {
            \Log::error('Error loading embed page: ' . $e->getMessage(), [
                'slug' => $slug,
                'user_id' => auth()->id(),
                'trace' => $e->getTraceAsString()
            ]);
